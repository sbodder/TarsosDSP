 ...
 * @author Joren Six
 */
public class AudioDispatcherControllable implements Runnable {
	 ...
	 * moment.
	 */
	private final UniversalAudioInputStream audioInputStream;
	 ...
	 */
	private final List<AudioProcessorControllable> audioProcessors;
	...
	private boolean runHasBeenCalled = false;
	...
	public AudioDispatcherControllable(final UniversalAudioInputStream stream, final int audioBufferSize, final int bufferOverlap){
		...
		audioProcessors = new CopyOnWriteArrayList<AudioProcessorControllable>();
	 ...
	 */
	public void addAudioProcessor(final AudioProcessorControllable audioProcessor) {
	 ...
	 */
	public void removeAudioProcessor(final AudioProcessorControllable audioProcessor) {
	...
	public void  pause()
	{
		...
		for (final AudioProcessorControllable processor : audioProcessors) {
					processor.pause();
		}
    ...
    public void seekTo(float seconds) throws IOException, InterruptedException {
	
		isEnded = false;
		if ( seconds > secondsProcessed() )
		{
			skip(seconds - secondsProcessed() ) ;
			skipToPoint();
		}
		else{
	...
	public void run() {
		...
		// As long as the stream has not been stopped
		while (!stopped) {
			...
			if (!isEnded){
				...
				if(!stopped && !isPaused){			
					...
					if (bytesRead==0){
						isEnded = true;
						// secProcessed = secondsProcessed();
						// audioInputStream.reset();
						// skip(secProcessed - 0.4);
						// skipToStart();
					}
				}
			}
